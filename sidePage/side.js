function addToCart(snackContainerID){
    // Adds the food item to the cart
    let snack = document.querySelector("#" + snackContainerID + " .snackTitle").innerHTML
    for(let i = 0; i < document.querySelector("#" + document.querySelector("#" + snackContainerID + " .quantityContainer").id + " .quantity").innerHTML; i++){
        localStorage.setItem(("cartArray" + (localStorage.length + 1)), "[\"" + "Side of " + snack + "\"]");
    }
    // resetsnackContainer();
}
...
function endAnimation(buttonID2, sizeID, containerID, quantityContainerID){
    ...
    console.log(document.getElementById(quantityContainerID))
